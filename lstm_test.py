...
X = np.genfromtxt(XFILE, delimiter=',')[:ROWS]
y = np.genfromtxt(YFILE, delimiter=',')[:ROWS]
...
def get_batch():
    ...
    seq = X.reshape((BATCH_SIZE, -1, INPUT_SIZE))
    # res = np.cos(xs)
    res = y.reshape((BATCH_SIZE, -1, OUTPUT_SIZE))
...
if __name__ == '__main__':
    ...
    for i in range(1000):
        ...
        # plt.draw()
        # plt.pause(0.3)
        pred = pred.reshape((ROWS, OUTPUT_SIZE))

        if i % 20 == 0:
            print('cost: ', round(cost, 4))
            # print('r2: ', round(r2_score()))
